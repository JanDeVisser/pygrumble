    ...
    def _float(coord, dir):
        if coord is None:
            ...
            try:
                ...
                return GeoPt._parse_coord(str(coord), dir)
    ...
    def _parse_coord(g, dir):
        ...
        if g:
            try:
                ...
                m = re.match(r"(-?[\d]+)\*?\s*([\d]+)\'?\s*([\d\.]+)\"?\s*([%s]?)" % dir, g)
                if m:
                    deg = int(m.group(1))
...
class GeoBox(object):
    ...
    def intersects(self, *other):
        ...
        if not(self and other):
            ...
            """
            return not(self.sw().lat <= other.sw().lat and 
                    self.sw().lon <= other.sw().lon and
                    self.ne().lat >= other.ne().lat and
                    self.ne().lat >= other.ne().lat)
