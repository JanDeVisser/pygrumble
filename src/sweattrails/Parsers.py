...
class CritPower():
	...
	for wp in self.waypoints[self.cur+1:i+1]:
	    ...
	    sum += wp.power if wp.power else 0
	avg = int(round(sum / self.duration))
	if avg > self.power:
	    self.power = avg
	    self.start = self.cur

    def persist(self, interval):
	cp = CriticalPower(parent = interval)
	cp.interval = interval
	cp.cpdef = self.cpdef
	cp.start_time = self.waypoints[self.start].timestamp
	cp.power = self.power
	cp.put()
	cp.updateBestCriticalPower()
...
class Analysis:
	...
	self.elev_loss = None
        ...
        if container.type == 'bike':
            for cpdef in CriticalPowerDef.gql("WHERE ANCESTOR IS :1", self.athlete):
    ...
    def analyze(self):
	logging.info(" --- Analyzing lap " + str(self.container.interval.interval_id) + " ---")
	sum_power = 0
	sum_torque = 0.0
	sum_cadence = 0
	sum_hr = 0
	...
	logging.info(" --- Saving lap " + str(interval.interval_id) + " --- ")
        if self.container.type == 'bike':
	...
	if self.max_lat or (self.elev_gain and self.elev_gain > 0):
	    ...
	    if self.max_lat:
		geodata.max_loc_ne = GeoPt(self.max_lat, self.max_lon)
		geodata.max_loc_sw = GeoPt(self.min_lat, self.min_lon)
	    geodata.put()
...
class IntervalContainer():
	...
	if self.wp is None:
		...
		self.wp.elapsed = self.ts
		self.wp.seconds = Util.time_to_seconds(self.ts)
	    ic = self
	    while ic:
		ic.waypoints.append(self.wp)
		ic = ic.parent
	return self.wp
...
class FileParser():
	...
	self.session = Session(parent=self.athlete)
	...
	logging.info(" --- FileParser.initialize_session() ---")
	
    def prepare(self):
	logging.info(" --- FileParser.prepare() ---")
...
def tcx_set_distance(context, cadence):
...
def tcx_set_latitude(context, lon):
